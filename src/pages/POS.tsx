                      ...
                      <TableCell className="py-1 text-sm font-black uppercase border-r border-slate-100 px-6 text-slate-800">
                        ...
                        {mode !== 'LOCACAO' && item.calculatorType && (
                          <div className="text-[10px] font-bold text-indigo-700 normal-case">
                            Calculadora: {item.calculationLabel || `calculado ${Number(item.calculatedQuantity || item.requestedQuantity || 0).toFixed(2).replace('.', ',')}`} • Venda: {Number(item.requestedQuantity || item.quantity || 0).toFixed(2).replace('.', ',')} {item.calculatorType === 'piso' ? 'm²' : 'un'}{item.boxSize > 0 ? ` • Arred.: ${item.boxesInput || Math.ceil(qty / boxSize).toString()}` : ''}
                          </div>
                        )}
      ...
      <Dialog open={isOtherAccountsAuthOpen} onOpenChange={setIsOtherAccountsAuthOpen}>
        <DialogContent className="max-w-md border-none shadow-2xl rounded-3xl">
          ...
          <form onSubmit={handleOtherAccountsAccess} className="space-y-5 py-4">
            ...
            <DialogFooter className="gap-3"><Button type="button" variant="outline" className="flex-1 h-14 rounded-2xl font-bold text-slate-500" onClick={() => setIsOtherAccountsAuthOpen(false)}>CANCELAR</Button><Button type="submit" className="flex-1 h-14 bg-indigo-600 hover:bg-indigo-700 rounded-2xl font-black text-lg shadow-xl shadow-indigo-500/20">LIBERAR</Button></form>
        </DialogContent>
      </Dialog>
            ...
            <DialogHeader>
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-2xl bg-emerald-500/15 text-emerald-300 border border-emerald-400/20 flex items-center justify-center shadow-inner<dyad-write path="src/pages/POS.tsx" overwrite={true} description="Reescrevendo o arquivo src/pages/POS.tsx por completo para garantir a correção do preço à vista, adicionar o filtro por data no caixa e habilitar a reimpressão direta das operações ao clicar no número do documento" isArtifact={false}>
"use client";
...
const POS = () => {
  ...
  return (
    <div className="h-screen w-screen bg-slate-200 flex overflow-hidden font-sans">
      ...
      <Dialog open={isDailyCashPanelOpen} onOpenChange={setIsDailyCashPanelOpen}>
        <DialogContent className="max-w-6xl h-[92vh] overflow-hidden rounded-3xl border-none shadow-2xl p-0 flex flex-col animate-in zoom-in-95">
          <div className="bg-slate-950 text-white p-6 shrink-0">
            <DialogHeader>
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                <div className="flex items-center gap-4">
      ...
      <PrintPreview isOpen={isPrintOpen} onClose={() => setIsPrintOpen(false)} data={lastActionData} type={printDocType} />

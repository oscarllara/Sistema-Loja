...
const POS = () => {
  ...
  const [paymentMethod, setPaymentMethod] = React.useState<'Dinheiro' | 'Cartão Crédito' | 'Cartão Débito' | 'PIX'>('Dinheiro');

  const products = db.produtos.getAll();
  const contas = db.contas.getAll();

  const addToCart = (product: any) => {
    const existing = cart.find(item => item.id === product.id || item.cd_produto === product.cd_produto);
    if (existing) {
      setCart(cart.map(item => 
        (item.id === product.id || item.cd_produto === product.cd_produto) ? { ...item, quantity: item.quantity + 1 } : item
      ));
    } else {
  ...
  const removeFromCart = (id: number) => {
    setCart(cart.filter(item => (item.id || item.cd_produto) !== id));
  };

  const updateQuantity = (id: number, delta: number) => {
    setCart(cart.map(item => {
      if ((item.id || item.cd_produto) === id) {
  ...
  const handleCheckout = () => {
    ...
    // Lógica de destino do dinheiro
    let contaDestino = contas.find(c => c.tipo === 'Caixa')?.cd_conta; // Padrão: Caixa Loja
    
    if (paymentMethod === 'PIX' || paymentMethod.includes('Cartão')) {
      contaDestino = contas.find(c => c.tipo === 'Banco')?.cd_conta || contaDestino;
    }
    ...
    try {
      ...
      db.financeiro.add({
        tipo: 'R',
        descricao: `Venda PDV - ${paymentMethod}`,
        valor: total,
        ...
        cd_conta: contaDestino
      });

      // 2. Baixa o estoque (simplificado)
      cart.forEach(item => {
      ...
      showSuccess(`Venda de R$ ${total.toFixed(2)} finalizada via ${paymentMethod}!`);
      setCart([]);
    } catch (err) {
                    ...
                    <div key={item.cd_produto} className="flex flex-col gap-2 pb-4 border-b border-slate-100 last:border-0">
                      <div className="flex justify-between items-start">
                        ...
                        <Button 
                          ...
                          onClick={() => removeFromCart(item.cd_produto)}
                        >
                          <Trash2 size={14} />
                        </Button>
                      </div>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-200 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2 pt-2">
                <Button 
                  ...
                  className={cn("flex flex-col h-16 gap-1", paymentMethod === 'Dinheiro' && "bg-indigo-600")}
                ...
                <Button 
                  variant={paymentMethod.includes('Cartão') ? 'default' : 'outline'} 
                  className={cn("flex flex-col h-16 gap-1", paymentMethod.includes('Cartão') && "bg-indigo-600")}
                ...
                <Button 
                  ...
                  className={cn("flex flex-col h-16 gap-1", paymentMethod === 'PIX' && "bg-indigo-600")}
              ...
              <Button 
                className="w-full h-14 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-lg font-bold shadow-lg"

...
const POS = () => {
  ...
  const [printDocType, setPrintDocType] = React.useState<'Venda' | 'Orcamento' | 'Fechamento' | 'Compra' | 'Locacao'>('Venda');
  ...
  const [dailyCashDate, setDailyCashDate] = React.useState(() => toLocalDateStr(new Date().toISOString()));
  ...
  const getProductPrice = (product: any, unit: string, currentPriceMode: 'PRAZO' | 'VISTA') => {
    ...
    const precoVista = typeof product.venda_vista === 'number' && product.venda_vista > 0 ? product.venda_vista : (product.venda || 0);
    ...
    try {
      ...
      const payload = {
        ...
        itens: cart.map(item => ({
          ...
          qtde: item.quantity,
          subtotal: Number((item.finalPrice * item.quantity).toFixed(2)),
  ...
  const dailyCashMovementsToday = React.useMemo(() => {
    ...
    return lancamentos.filter(l => {
      ...
      const date = toLocalDateStr(l.data_pagamento || l.data_vencimento);
      return date === dailyCashDate;
    });
  }, [dailyCashAccount, lancamentos, dailyCashDate]);
  ...
  const handleDailyCashAccess = (e: React.FormEvent) => {
    ...
    setDailyCashPassword("");
    setDailyCashDate(toLocalDateStr(new Date().toISOString()));
  ...
  const handleDailyCashReprintSale = async (item: LancamentoFinanceiro) => {
    await handleReprintDocument(item);
  };
    ...
    try {
      if (doc.type === 'Venda') {
        ...
        if (compra) {
          ...
          setLastActionData({ ...compra, type: 'Compra' });
          setIsPrintOpen(true);
        } else {
        ...
        if (rental) {
          ...
          setLastActionData({ ...rental, type: 'Locacao' });
          setIsPrintOpen(true);
        } else {
  ...
  return (
    <div className="h-screen w-screen bg-slate-200 flex overflow-hidden font-sans">
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-xs font-black text-slate-900 uppercase">{cashAccount?.nome || 'Sem caixa cadastrado'}</p>
              </div>
              <span className={cn(
      ...
      <Dialog open={isCalculatorOpen} onOpenChange={setIsCalculatorOpen}>
        <DialogContent className="max-w-6xl max-h-[92vh] overflow-y-auto rounded-3xl border-none shadow-2xl p-0">
        ...
        <DialogContent className="max-w-md rounded-3xl">
          <DialogHeader>
            <DialogTitle className="text-xl font-black uppercase tracking-tighter flex items-center gap-2">
              ...
              Acesso de Supervisor / Gerente
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleDailyCashAccess} className="space-y-5 py-4">
            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Senha de Supervisor ou Gerente</Label>
              <Input 
                type="password" 
                autoFocus 
                value={dailyCashPassword} 
                onChange={(e) => setDailyCashPassword(e.target.value)} 
                className="h-14 text-2xl font-black border-2 border-slate-200 focus:border-emerald-500 rounded-2xl shadow-inner" 
                placeholder="••••••" 
              />
            </div>
            <DialogFooter className="gap-3">
              <Button type="button" variant="outline" className="flex-1 h-12 rounded-xl font-bold" onClick={() => setIsDailyCashAuthOpen(false)}>CANCELAR</Button>
              <Button type="submit" className="flex-1 h-12 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-black">ENTRAR NO CAIXA</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isOtherAccountsAuthOpen} onOpenChange={setIsOtherAccountsAuthOpen}>
        <DialogContent className="max-w-md border-none shadow-2xl rounded-3xl">
          ...
          <form onSubmit={handleOtherAccountsAccess} className="space-y-5 py-4">
            ...
            <DialogFooter className="gap-3"><Button type="button" variant="outline" className="flex-1 h-14 rounded-2xl font-bold text-slate-500" onClick={() => setIsOtherAccountsAuthOpen(false)}>CANCELAR</Button><Button type="submit" className="flex-1 h-14 bg-indigo-600 hover:bg-indigo-700 rounded-2xl font-black text-lg shadow-xl shadow-indigo-500/20">LIBERAR</Button></DialogFooter></form>
        </DialogContent>
      </Dialog>
          ...
          <div className="bg-slate-950 text-white p-6 shrink-0">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400 font-bold mt-1">
                      <span>{dailyCashAccount?.nome || 'Sem conta'} • {dailyCashAccount?.tipo || 'Conta'} • {isDailyCashMainAccount ? (currentCashSession?.status || 'Fechado') : 'Consulta Admin'}</span>
                      <div className="w-px h-3 bg-white/20" />
                      <div className="flex items-center gap-1.5 bg-white/10 text-white px-2 py-1 rounded-lg">
                        <CalendarIcon size={12} className="text-emerald-400" />
                        <input
                          type="date"
                          value={dailyCashDate}
                          onChange={(e) => setDailyCashDate(e.target.value)}
                          className="bg-transparent border-none p-0 focus:ring-0 text-xs font-black cursor-pointer text-white"
                        />
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-6">
              <button type="button" className={cn("text-left rounded-2xl bg-white/10 border border-white/10 p-4 transition-all hover:bg-white/15", dailyCashFilter === 'Todos' && dailyCashTypeFilter === 'Todos' && "ring-2 ring-white/30")} onClick={() => { setDailyCashFilter('Todos'); setDailyCashTypeFilter('Todos'); }}>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Abertura / Saldo Inicial</p>
              ...
              <button type="button" className="text-left rounded-2xl bg-indigo-500/10 border border-indigo-400/20 p-4 transition-all hover:bg-indigo-500/15" onClick={() => { setDailyCashFilter('Todos'); setDailyCashTypeFilter('Todos'); }}>
                <p className="text-[10px] font-black uppercase tracking-widest text-indigo-300">Saldo Final / Acumulado</p>
              ...
              <div className="p-4 border-b border-slate-100 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                    Exibindo dia: {new Date(`${dailyCashDate}T00:00:00`).toLocaleDateString('pt-BR')} • Filtro: {dailyCashFilter}{dailyCashTypeFilter !== 'Todos' ? ` • ${dailyCashTypeFilter === 'R' ? 'Entradas' : 'Saídas'}` : ''}
                  </p>
                </div>
                ...
                <Table>
                  <TableHeader className="sticky top-0 bg-white z-10">
                    <TableRow>
                      ...
                      <TableHead className="text-[10px] font-black uppercase">Descrição / Documento</TableHead>
                    ...
                    {filteredDailyCashMovements.length === 0 && !showDailyCashInitialRow ? (
                      ...
                      <>
                        ...
                        {filteredDailyCashMovements.map(item => {
                          ...
                          const descriptionWithoutSale = doc
      ...
      <SalesHistoryModal isOpen={isHistoryOpen} onClose={() => setIsHistoryOpen(false)} onReprint={(v) => { setLastActionData({ ...v, type: 'Venda' }); setPrintDocType('Venda'); setIsPrintOpen(true); }} mode={mode} />
      ...
      <PrintPreview isOpen={isPrintOpen} onClose={() => setIsPrintOpen(false)} data={lastActionData} type={printDocType as any} />
      ...
      <Dialog open={isAdminAuthOpen} onOpenChange={setIsAdminAuthOpen}><DialogContent className="max-w-md rounded-3xl"><DialogHeader><DialogTitle className="text-xl font-black uppercase tracking-tighter">Acesso Restrito ao ERP</DialogTitle></DialogHeader><form onSubmit={(e) => { e.preventDefault(); const isValid = adminPassword === 'admin' || adminPassword === 'Senha@123' || sellers.some(s => s.senha === adminPassword && (s.usuario?.includes('admin') || s.permissoes?.dashboard)); if (isValid) { navigate("/dashboard"); } else { showError("Senha incorreta."); } }} className="space-y-5 py-4"><div className="space-y-2"><Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Senha do Administrador</Label><Input type="password" autoFocus value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} className="h-14 text-2xl font-black border-2 border-slate-200 focus:border-primary rounded-2xl shadow-inner" placeholder="••••••" /></div><DialogFooter className="gap-3"><Button type="button" variant="outline" className="flex-1 h-12 rounded-xl font-bold" onClick={() => setIsAdminAuthOpen(false)}>CANCELAR</Button><Button type="submit" className="flex-1 h-12 bg-slate-900 hover:bg-black text-white rounded-xl font-black">ACESSAR ERP</Button></DialogFooter></form></DialogContent></Dialog>
    </div>
  );
};
